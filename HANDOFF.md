...
## 1.5 🛑 协作规则（**第 2 天结束时用户新提的硬要求，优先级最高**）

> ⚠️ **第 2 天犯过的错，别再犯**：agent 直接把 `index.html` / `css/style.css` / `js/script.js`
> 三个文件全写完了，用户只是在旁边看。用户明确表示**这样学不到东西**。
> 用户要的是「练手 + 作品集」，不是「拿到能跑的代码」。

### 铁律
1. **代码必须用户自己写。** agent 的产出是「思路 + 知识点讲解 + hints + 检查点 + 报错分析」，
   **不是可用的代码。**
2. **不要主动用 write / edit 去改** `index.html`、`css/style.css`、`js/script.js`。
   用户没让你改，就别改。想演示也只在聊天里贴**片段**，并且先说清"只是示意，你要自己写一遍"。
3. **唯一允许 agent 直接改的文件**：`HANDOFF.md`（交接文档本身）。
4. **agent 可以跑 `git add` / `git commit`**（用户不介意），但 `git push` 必须用户手动
   （沙箱无外网，见第 9 节）。
5. **一次只推进一天**，讲完知识点后**停下来等用户回报**，别一口气把一天的活讲完。

### hints 的分级策略（用户原话整理）
用户希望**先介绍需要用到什么知识点**，然后按他的掌握程度分两种处理：

| 情况 | agent 该怎么做 |
|---|---|
| **用户学过的知识**（HTML/CSS/JS 基础、Python/C++ 基础） | 只给**思路**：说清"要实现什么效果"+"该用哪个 API"，代码他自己写 |
| **用户遗忘或没学过的** | 给**知识点讲解 + hints**，讲透原理 |
| **大块的新知识**（如某个完整概念） | 提供**外部学习网站链接**（优先中文，如 MDN 中文、现代 JS 教程） |
| **小块的新知识**（如某个方法怎么用） | **直接在聊天里讲解**，不要甩链接 |

> 用户还说：**「主要是了解知识框架，对于前端技术的掌握程度不需要太高，一些必要性的代码让我自己来。」**
> 意思是——他不需要成为前端专家，但**核心逻辑必须自己写**。
> 所以别把时间花在 CSS 像素级打磨上，那部分可以 agent 代劳或直接给成品；
> 但**逻辑部分（洗牌、翻牌状态、配对判定、计时）必须留给他。**

### 用户的知识框架要求
用户想**先知道整体知识框架**（做这个项目要用到哪些技能点），再动手。
所以每天开工前，先给一个「今天要用到的知识点清单 + 属于哪一档」，让他自己认领。
